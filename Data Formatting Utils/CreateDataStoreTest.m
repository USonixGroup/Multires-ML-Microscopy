...
A=readstruct("livecell_coco_test.json"); %file containing json data
...
for i=[1:length(counts)] %for the number of images
    ...
    im = imread( ["./LiveCellsIms1/livecell_test_images/"+FileNamesIDS{i, 2   } ]   ); %change with image path as needed
        % if size(im, 3)==1
