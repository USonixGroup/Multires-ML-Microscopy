/*
 *  EDGECANNY_AUTOTH_R_RT_RM Helper function for Edge block (Canny method).
 *
 *  Copyright 2019 The MathWorks, Inc.
 */
#include "vipedge_rt.h"

static void TrackNonRecursive_AR_RM ( boolean_T *edge,
                                real32_T  *tmpOrMag,
                                int16_T   *stackR,
                                int16_T   *stackC,
                                int16_T   *stackP,
                ...
                for (p= -1; p<=1; p++)
                {
                    for(q= -1; q<=1; q++)
                    {
                        ...
                        if (isInRange(R+p, C+q, inpRows, inpCols) &&
                                tmpOrMag[(R+p)*inpCols + (C+q)] >= lowTh)
                        {
                            stackIdx++;
...
static void EstimateAutoThreshold_R_RM (real32_T *tmpOrMag,
                                    real32_T *highTh,
                                    real32_T *lowTh,
                                    int_T inpRows,
                                    int_T inpCols,
                                    ...
                                    const real32_T *autoPercent)
{
    ...
    for (r=quarterFiltLen; r<inpRows-quarterFiltLen; r++)
        ...
        {
            real32_T valBasedIdx = (tmpOrMag[r*inpCols+c] * N_BINS_MIN1);
...
static void HysteresisThresholding_AR_RM (boolean_T *edge,
                                        real32_T  *tmpOrMag,
                                        int16_T   *stackR,
                                        int16_T   *stackC,
                                        int16_T   *stackP,
                                        ...
                                        const real32_T  *autoPercent)
{
    ...
    EstimateAutoThreshold_R_RM (tmpOrMag, &highTh, &lowTh,inpRows,
            inpCols,quarterFiltLen,autoPercent);
    
    /*stack based Track Non-Recursive function is replaced by following function with if else condition*/
    for (r=0; r<inpRows; r++)
        for (c=0; c<inpCols; c++)
            if (tmpOrMag[r*inpCols + c] >= highTh)
                TrackNonRecursive_AR_RM (edge,tmpOrMag,stackR,stackC,stackP,stackQ,
                        r, c, lowTh, inpRows,inpCols);
}


LIBMWVISIONRT_API void MWVIP_EdgeCanny_autoTh_R_RM(
                                                ...
                                                int_T  halfFiltLen)
{
    ...
    MWVIP_RC_Gaussian_Smoothing_R_RM(inpImg,gauss1D,cFiltered,rFiltered,inpRows,inpCols,halfFiltLen);
    
    /* step-2: filter smoothed image along column with the derivative of Gaussian filter */
    MWVIP_C_Derivative_Image_R_RM(cFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    ...
    MWVIP_R_Derivative_Image_R_RM(rFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    ...
    MWVIP_NonMaximum_Suppression_R_RM(cFiltered,rFiltered,tmpOrMag,inpRows,inpCols);
    ...
    HysteresisThresholding_AR_RM (outEdge,tmpOrMag,stackR,stackC,stackP,stackQ,
...
/* [EOF] edgecanny_autoth_r_rt_rm.c */
