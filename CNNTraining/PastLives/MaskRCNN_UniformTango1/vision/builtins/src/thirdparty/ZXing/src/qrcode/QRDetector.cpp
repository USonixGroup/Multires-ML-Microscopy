...
*/
AlignmentPattern FindAlignmentInRegion(const BitMatrix& image, float overallEstModuleSize, int estAlignmentX, int estAlignmentY, float allowanceFactor, const bool isStringent = false)
{
	...
	return AlignmentPatternFinder::Find(image, alignmentAreaLeftX, alignmentAreaTopY, alignmentAreaRightX - alignmentAreaLeftX, alignmentAreaBottomY - alignmentAreaTopY, overallEstModuleSize, isStringent);
}
...
ProcessFinderPatternInfo(const BitMatrix& image, const FinderPatternInfo& info)
{
	...
	// Anything above version 1 has an alignment pattern except for Model 1 which has no alignment pattern
	if (!provisionalVersion->alignmentPatternCenters().empty()) {
		...
		for (int i = 4; i <= 16; i <<= 1) {
			// The last argument for FindAlignmentInRegion is "bool isStringent". This change was introduced when support for Model 1 was added.
			// Model 1 do not contain alignment patterns and were causing false detections which the more stringent check eliminates.
			// If isStringent = FALSE, this function searches for an alignment pattern in the image using the original algorithm found in ZXing.
			// The original algorithm checks only the horizontal and vertical space around potential alignment patterns to confirm the appropriate pattern is present.
			// If isStringent = TRUE, the NW-SE and NE-SW diagonal spaces around the potential alignment patterns are also checked.
			// The original algorithm is used when the search radius is at its smallest, 4 modules, to maintain ZXing's standard sensitivity when the pattern is close to the expected location.
			// When the search radius increases, the probability of finding a false positive with the original algorithm increases, so the more stringent algorithm is used.
			alignmentPattern = FindAlignmentInRegion(image, moduleSize, estAlignmentX, estAlignmentY, static_cast<float>(i), i == 4);
			if (alignmentPattern.isValid()) {
				break;
			}
		}
	...
	if (bits.empty()) {
		// This section was introduced when support for Model 1 was added.
		// bits is initialized empty if the transform provided to the "sampleGrid" function is invalid.
		// The invalid transform could be caused by a false positive alignment pattern (see comment on line 311).
		alignmentPattern.makeInvalid();

		// Generate the default transform with no alignment pattern. This produces an identity matrix transform.
		PerspectiveTransform defaultTransform = CreateTransform(info.topLeft, info.topRight, info.bottomLeft, alignmentPattern, dimension);
		bits = GridSampler::Instance()->sampleGrid(image, dimension, dimension, defaultTransform);
	}
