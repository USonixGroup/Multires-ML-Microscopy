...
const Version* BitMatrixParser::ReadVersionModel1(const BitMatrix& bitMatrix, bool mirrored)
{
    (void)mirrored; //silence unreferenced warning
    if (!hasValidDimension(bitMatrix)) {
		return nullptr;
	}

    auto parsedVersion = Version::VersionForDimensionModel1(bitMatrix.width());
...
* QR Code Model 1.
* 
* This method is based on the ZXing Model 1 fork (Java), readCodewordsModel1: 
* https://github.com/george-hopkins/zxing/blob/qrmodel1/core/src/main/java/com/google/zxing/qrcode/decoder/BitMatrixParser.java</p>
*
* @return bytes encoded within the QR Code Model 1
* @throws FormatException if the exact number of bytes expected is not read
*/
ByteArray
BitMatrixParser::ReadCodewordsModel1(const BitMatrix& bitMatrix, const Version& version)
{
	if (!hasValidDimension(bitMatrix)) {
		return {};
	}
	...
	for (int j = 0; j < columns; j++) {
		if (j <= 1) { // vertical symbols on the right side
			...
			for (int i = 0; i < rows; i++) {
				...
				result[resultOffset++] = static_cast<uint8_t>(d);
			}
