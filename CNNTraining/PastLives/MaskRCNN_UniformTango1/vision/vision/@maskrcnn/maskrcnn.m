classdef maskrcnn < deep.internal.sdk.LearnableParameterContainer

% Copyright 2021-2023 The MathWorks, Inc.

    properties (SetAccess = public, GetAccess = {?tmaskrcnn, ?ttrainMaskRCNN})
    ...
    properties(SetAccess=public, GetAccess = ?tmaskrcnn)
        PoolSize = [14 14];
        MaskPoolSize = [14 14];
    end
    ...
    % Constructors
    methods
        function obj = maskrcnn(detector, classNames, anchorBoxes, options)
            arguments
                detector {mustBeMember(detector, {'resnet50-coco','none'})}
                ...
                options.ModelName {mustBeTextScalar} = 'maskrcnn'
            end
            ...
            % Check if Mask R-CNN spkg is installed
            spkgroot = iTripwireMaskRCNN();

            % Load pre-trained network
            data = iLoadNetwork(spkgroot,"LargeData","dlnetPostFeature.mat");
            obj.PostPoolFeatureExtractionNet = data.dlnetPostFeature;
            data = iLoadNetwork(spkgroot,"data","dlnetFeature.mat");
            obj.FeatureExtractionNet = data.dlnetFeature;
            data = iLoadNetwork(spkgroot,"data","dlnetRPN.mat" );
            obj.RegionProposalNet = data.dlnetRPN;
            data = iLoadNetwork(spkgroot,"data","dlnetDetectHead.mat" );
            obj.DetectionHeads = data.dlnetDetectHead;
            data = iLoadNetwork(spkgroot,"data","dlnetMaskHead.mat" );
    ...
    methods(Access=public)
        ...
        function outputFeatures = predict(obj, dlX)
            ...
            dlFeatures = predict(obj.FeatureExtractionNet, dlX, 'Acceleration','auto', OutputDataFormats='SSCB');
    
            [dlRPNScores, dlRPNReg] = predict(obj.RegionProposalNet, dlFeatures, 'Outputs',{'RPNClassOut', 'RPNRegOut'});
            
            %redundant, added a dl layer instead
            % dlRPNScores=softmax(dlRPNScores); %custom: softmax to ensure RPN predictions are not small numbers
            ...
            [dlBoxReg, dlBoxScores] = predict(obj.DetectionHeads, dlFinalFeatures, 'Acceleration','auto');
    ...
    methods(Access=public)
        ...
        function [dlRPNScores,dlRPNReg,dlProposals,dlBoxScores,dlBoxReg,dlMasks,state] = forward(obj, dlX ,varargin)
            ...
            [dlBoxReg, dlBoxScores, localState] = forward(obj.DetectionHeads, dlFinalFeatures, 'Acceleration','none');
            state = vertcat(state, localState);
        ...
        function [masks, boxLabel, boxScore, boxes] = segmentObjectsInBatch(obj, im, executionEnvironment)
            ...
            for batchIdx = 1:batchSize
                ...
                if(iscell(im))
                    ...
                    [imResized(:,:,:,batchIdx), scaleRatio(batchIdx)] = preprocessImage(obj, single(im{batchIdx}), obj.InputSize, obj.ImageMean); %#ok<AGROW> 
                else
