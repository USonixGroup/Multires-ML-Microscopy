...
elseif (strcmpi(ext,'pcd'))
    ...
    if ~isempty(ptCloud.Color)
        coder.internal.errorIf(isa(ptCloud.Color, "uint16"), ...
            'vision:pointcloud:uint16ColorsNotSupported')
        
        % Supports the color information in the PCD standard rgb format
        % If the color information needs to be written separately, please
...
parser.addRequired('ptCloud', @(x)validateattributes(x, {'pointCloud'},{'scalar'}));
