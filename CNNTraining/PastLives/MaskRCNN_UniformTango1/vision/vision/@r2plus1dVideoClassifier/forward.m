function varargout = forward(this,dlVideo)
%forward  Compute R(2+1)D video classifier outputs for training.
%
%   dlYVideo = forward(rd, dlXVideo) returns R(2+1)D video classifier
%   outputs for training. The input rd is a r2plus1dVideoClassifier object.
%   The input dlXVideo is a dlarray object that corresponds to the video
%   input to the classifier. The size of the input must be
...
%   network.
%
%   [dlYVideo,stateVideo] = forward(rd, dlXVideo) also returns the updated
...
%
%   Example: Forward video using R(2+1)D for training.
%   --------------------------------------------------
...
%   activations = forward(rd,dlVideo);
%
% See also r2plus1dVideoClassifier/predict, slowFastVideoClassifier,
...
%   Copyright 2021-2023 The MathWorks, Inc.
    ...
    arguments
        ...
        dlVideo {mustBeA(dlVideo,"dlarray"),iValidateInputFrames(dlVideo)}
    end
    % Convert Time to spatial.
    dlVideo = dlarray(dlVideo, "SSCBS");
    [varargout{1:nargout}] = this.Network.forward(dlVideo);
end
