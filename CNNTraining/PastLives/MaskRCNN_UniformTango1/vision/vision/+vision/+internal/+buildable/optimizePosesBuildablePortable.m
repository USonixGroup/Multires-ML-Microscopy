classdef optimizePosesBuildablePortable < coder.ExternalDependency %#codegen
    % optimizePosesBuildablePortable - Buildable for portable codegen
    % support for optimizePoses

    % Copyright 2023 The MathWorks, Inc.
    methods (Static)

        function name = getDescriptiveName(~)
            name = 'optimizePosesBuildablePortable';
        end
        ...
        function [optimPoses, optimStruct, optimPoseScales] = visionOptimizePoses(params)
            ...
            coder.inline('always');
            coder.cinclude('optimizePoses_published_c_api.hpp');
            ...
            for i=1:numEdges
                ...
                if(isSimilar)
                    infoMatCompact = vision.internal.buildable.optimizePosesBuildablePortable.compactifySimilarityInformationMatrix(info{i});
                else
                    infoMatCompact = vision.internal.buildable.optimizePosesBuildablePortable.compactifyRigidInformationMatrix(info{i});
                end
