classdef kdtreeBuildablePortable < coder.ExternalDependency
% kdtreeBuildablePortable - encapsulate kdtree portable code implementation library
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo,~)
            ...
            buildInfo.addIncludePaths({fullfile(matlabroot,'toolbox', ...
                'vision','builtins','src','shared','kdtree','export','include','kdtree'),...
                fullfile(matlabroot,'toolbox', ...
                'vision','builtins','src','shared','kdtree','export','include')});
                
            srcPaths = ({fullfile(matlabroot,'toolbox', ...
                'vision','builtins','src','shared','kdtree')});
            buildInfo.addSourceFiles({'kdtree.cpp'},srcPaths);                

            kdtreeHeaderPath = fullfile(matlabroot,'toolbox', ...
                'vision','builtins','src','shared','kdtree','export', ...
                'include','kdtree');

            buildInfo.addIncludeFiles({'vision_defines.h','HybridSearchLoopImpl.hpp', ...
                'KdtreeImpl.hpp','KdtreeUtil.hpp','KnnSearchLoopImpl.hpp', ...
                'nanoflann.hpp','RadiusSearchLoopImpl.hpp','kdtree_published_c_api.hpp'}, ...
                kdtreeHeaderPath);
        end
        ...
        function ptrObj = kdtreeConstruct(dataType)
            ...
            coder.inline('always');
            coder.cinclude('kdtree_published_c_api.hpp');
