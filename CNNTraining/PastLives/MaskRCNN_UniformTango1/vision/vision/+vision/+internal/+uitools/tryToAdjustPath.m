...
% systems.
%
% tryToAdjustPath(..., 'FileNotFoundError', TF) optionally specify true
% or false if function should error if file is not found. If this is false,
% the output newPath will be empty.
function newPath = tryToAdjustPath(origPath, currentSessionFilePath,...
    origFullSessionFileName,varargin)
...
% parameters
parser = inputParser;
parser.addParameter('FileNotFoundError', true, @islogical);
...
%--------------------------------------------------------------
    function path = getRelativePath(oldPath)
        ...
        % Even though current path contains imageFilepath pattern but
        % there is no relative path. 
        % Ex: pattern = 'C:\' str = 'C:\User\username\Desktop'
        if idx <= 0
            idx = [];
        end
