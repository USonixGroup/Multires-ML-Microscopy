...
%%
load("/home/zcemydo/Scratch/TrainDWT/DWTNet.mat")
...
imageSize=[520 704 4];
 ...
 ds = fileDatastore([datdir+"/CatDSFs"], ReadFcn=@(x)matreaderDWT(x)); %training datatrainDS = transform(ds, @(x)helper.preprocessData(x, imageSize));
...
savefreq= 800; %iterations, move location in for loop to save every X epoch
disp(params)
    ...
    mb.shuffle();
        ...
        while mb.hasdata()
            ...
            if rem(numIteration, savefreq) == 0
                    ...
                    save("~/Scratch/TrainDWT/NetData101/Checkpoint-"+modelDateTime+".mat", "savenet"); %save output with the date and time into the current directotry
            end
...
dlnet=gather(dlnet);
modelDateTime = string(datetime("now",Format="yyyy-MM-dd-HH-mm-ss"));
                    save("~/Scratch/TrainDWT/NetData101/FINAL"+modelDateTime+".mat"); %save output with the date and time into the current directotry
