...
datdir=".";
 ds = fileDatastore([datdir+"/newcircs"], ReadFcn=@(x)MATReader1C(x)); %training datatrainDS = transform(ds, @(x)helper.preprocessData(x, imageSize));
...
savefreq= 600; %iterations, move location in for loop to save every X epoch
% params.OverlapThreshold=0.1;
% params.NumStrongestRegions=300;
% params.RPNROIPerImage=300;
% params.StandardizeRegressionTargets=1;
% params.MiniBatchPadWithNegatives=0;
...
if doTraining
    ...
    mb.shuffle();
        ...
        while mb.hasdata()
            ...
            learnRate = initialLearnRate*0.95^numIteration;

            disp('LR: '+string(learnRate) )
            ...
            if rem(numIteration, savefreq) == 0
                    ...
                    %save("~/Scratch/Train1C/NetData101/Checkpoint-"+modelDateTime+".mat", "savenet"); %save output with the date and time into the current directotry
            end
