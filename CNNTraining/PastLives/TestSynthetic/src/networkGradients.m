...
RPNRegDeltas = {'rpnConv1x1BoxDeltas'};regionProposal = {'rpl'};
 %outs={'rcnnSoftmax'	'fcBoxDeltas'	'rpnSoftmax'	'mask_sigmoid1'}

outputNodes = [ RPNRegDeltas, regionProposal, 'rcnnSoftmax'	'fcBoxDeltas'	'rpnSoftmax'	'mask_sigmoid1'];
...
classNames = categories(categorical((params.ClassNames(1:end-1)))); %all class names except the background, which is in the last position
...
% onehotencode labels
classificationTargets = onehotencode(classificationTargets,1, ClassNames=params.ClassNames);
...
LossRPN = LossRPNClass + LossRPNReg;
 ...
 disp( ["RPNClass "+string(LossRPNClass)+', RPNReg '+string(LossRPNReg)+', RCNNReg '+string(LossRCNNReg)+', RCNNClass '+string(LossRCNNClass)+', RCNNMask '+string(LossRCNNMask)] )
