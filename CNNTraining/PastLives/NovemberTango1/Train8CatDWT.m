...
load("~/Scratch/NewTrain1/Cat8Channel4.mat")

datdir="~/Scratch/CatDSFs";
 
ds = fileDatastore([datdir], ReadFcn=@(x)matreaderDWT(x)); %training datatrainDS = transform(ds, @(x)helper.preprocessData(x, imageSize));
    ...
    mb.shuffle();
        ...
        while mb.hasdata()
            ...
            if rem(numIteration, savefreq) == 0
                    ...
                    save("~/Scratch/NewTrain1/NetData101/Cat8DWT-Checkpoint-"+modelDateTime+".mat", "savenet"); %save output with the date and time into the current directotry
            end
