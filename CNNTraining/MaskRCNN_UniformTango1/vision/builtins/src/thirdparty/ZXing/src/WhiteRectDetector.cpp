...
bool WhiteRectDetector::Detect(const BitMatrix& image, int initSize, int x, int y, ResultPoint& p0, ResultPoint& p1, ResultPoint& p2, ResultPoint& p3)
{
	...
	if (!sizeExceeded && atLeastOneBlackPointFoundOnBorder) {
		...
		//go down left
		for (int i = 1; !found && i < maxSize; i++) {
			found = GetBlackPointOnSegment(image, right, up + i, right - i, up, x1);
		}
		...
		//go up left
		for (int i = 1; !found && i < maxSize; i++) {
			found = GetBlackPointOnSegment(image, right, down - i, right - i, down, y1);
		}
		...
		CenterEdges(y1, z, x1, t, width, p0, p1, p2, p3);
		return true;
	}
	else {
		return false;
