...
struct CallOnceFlag
{
    enum { INIT, RUNNING, DONE } state;
    std::mutex mut;
    std::condition_variable cv;
    CallOnceFlag() : state(INIT) {}
};   
...
// Call a void(...) function only once
template<typename Callable, typename... Args>
static void CallOnce(CallOnceFlag &flag, Callable &&f, Args&&... args)
{

    typedef std::unique_lock<std::mutex> UniqueLock;

    {
        UniqueLock lock(flag.mut);
        flag.cv.wait(lock, [&flag]() { return flag.state != flag.RUNNING; });
...
{
	CallOnce(_cache->callonce, &InitBlackMatrix, std::cref(*_source), std::ref(_cache->matrix));   
