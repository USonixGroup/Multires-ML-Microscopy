...
enum { SHAPE_RECTANGLE = 1, SHAPE_LINE = 2, SHAPE_POLYGON = 3, SHAPE_CIRCLE = 4 };
...
enum { MARKER_CIRCLE = 1, MARKER_XMARK = 2, MARKER_PLUS = 3, MARKER_STAR = 4, MARKER_SQUARE = 5 };
...
boolean_T useFillShapePolygonCodeX(const int32_T shape,
                                   boolean_T isFillShape,
                                   boolean_T mimicFillPoly);

boolean_T isLineOrNonFilledPolygon(const int32_T shape, boolean_T isFillShape);

void drawShapes(DrawBase** drawBaseP2P,
                boolean_T isFillShape,
...
template <typename T>
boolean_T CheckDataDimen(T numDimen, const T* dimens, T dimLen, T& numCount) {
...
template <typename T>
boolean_T CheckDataDimenEven(T numDimen,
                             const T* dimens,
                             T minNumRow,
                             T& numCount,
                             T& numSubCount) {
...
template <typename T>
boolean_T CheckAndGetNumShape(int32_T shape,
                              T numDimen,
                              const T* dimens,
                              int_T* pNumShape = NULL,
                              int_T* pNumSubShape = NULL) {
    boolean_T isValid = false;
    int_T numShape = 0;
    int_T numSubShape = 0;
...
template <typename T>
boolean_T CheckAndGetNumMarker(int32_T shape,
                               T numDimen,
                               const T* dimens,
                               int_T* pNumShape = NULL,
                               int_T* pNumSubShape = NULL) {
...
DrawBase* InstantiateShape(DT* rgbOut,
                           PT* pts,
                           DT* fillColor,
                           OpDT* opacity,
    ...
    DT* imageG = image + (numCol * numRow * 1); // Pointer to first element of green channel of
                                                // image
    ...
    if (shape == SHAPE_RECTANGLE) {
        if (isFillShape) {
            drawBase =
                (DrawBase*)new DrawFilledRectangleAnySize<DT, BT, PT, VT, OpDT, OpBT,
                                                          MODE_PORT_ONE>(DRAW_COMM_VALUE_LIST, pts);
        } else {
            drawBase =
                (DrawBase*)new DrawRectangleAnySize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE>(
    ...
    // Instantiate class for Thick line or Polygon
        ...
        if (useFillShapePolygonCodeX(shape, isFillShape, mimicFillPoly)) {
            ptsOrPtsDW =
                (isLineOrNonFilledPolygon(shape, isFillShape) && mimicFillPoly) ? ptsDW : pts;
            ...
            if (isAntiAlias) {
                ...
                Polygon<int32_T, PT, MODE_ALG_ANTIALIAS>* polygon =
                    new Polygon<int32_T, PT, MODE_ALG_ANTIALIAS>(
                        ptsOrPtsDW, ptsDW, polygonEdge, polygonEdgeBk, numShape, numPtPerPolygon,
                        numShape * numPtPerPolygon, foldPtDW);

                ScanLineBase<int32_T, PT, MODE_ALG_ANTIALIAS>* scanline =
                    (ScanLineBase<int32_T, PT, MODE_ALG_ANTIALIAS>*)new ScanLine<
                        int32_T, PT, MODE_ALG_ANTIALIAS>(polygon, 0, 0, numRow * SUBPIX_LEN - 1,
                                                         numCol * SUBPIX_LEN - 1);

                drawBase = (DrawBase*)new DrawFilledPolygon<DT, BT, PT, VT, OpDT, OpBT,
                                                            MODE_PORT_ONE, MODE_ALG_ANTIALIAS>(
                    DRAW_COMM_VALUE_LIST, scanline, pts, ptsDW);
            } else {

                Polygon<int32_T, PT, MODE_ALG_DIRECT>* polygon =
                    new Polygon<int32_T, PT, MODE_ALG_DIRECT>(
                        ptsOrPtsDW, ptsDW, polygonEdge, polygonEdgeBk, numShape, numPtPerPolygon,
                        numShape * numPtPerPolygon, foldPtDW);

                ScanLineBase<int32_T, PT, MODE_ALG_DIRECT>* scanline =
                    (ScanLineBase<int32_T, PT, MODE_ALG_DIRECT>*)new ScanLine<int32_T, PT,
                                                                              MODE_ALG_DIRECT>(
                        polygon, 0, 0, numRow - 1, numCol - 1);

                drawBase = (DrawBase*)new DrawFilledPolygon<DT, BT, PT, VT, OpDT, OpBT,
                                                            MODE_PORT_ONE, MODE_ALG_DIRECT>(
                    DRAW_COMM_VALUE_LIST, scanline, pts, ptsDW);
            }
        }
            ...
            if (isAntiAlias) {
                ...
                drawBase = (DrawBase*)new DrawPolygon<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                      MODE_ALG_ANTIALIAS>(
    ...
    // Instantiate class for Thin Line (Line Width < 1)
        ...
        if (isAntiAlias) {
            drawBase = (DrawBase*)new DrawAnyLine<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                  MODE_ALG_ANTIALIAS>(
                DRAW_COMM_VALUE_LIST, pts, ptsDW); // remove ptsDW from here
        } else {
    ...
    // Instantiate class for Circle
        ...
        if (isAntiAlias) {
            if (isFillShape) {
                drawBase =
                    (DrawBase*)new DrawCircleAnySize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
            ...
            if (isFillShape) {
                ...
                drawBase =
                    (DrawBase*)new DrawCircleAnySize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                     MODE_INT_EMPTY, MODE_ALG_DIRECT>(
                        DRAW_COMM_VALUE_LIST, pts);
            }
        }
    }
...
template <class DT, class BT, class PT, class VT, class OpDT, class OpBT>
DrawBase* InstantiateMarker(DT* rgbOut,
                            PT* pts,
                            OpDT* opacity,
                            DT* fillColor,
                            int32_T markerShape,
    ...
    case MARKER_SQUARE:
        drawBase = (DrawBase*)new DrawSquareFixedSize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE>(
            DRAW_COMM_VALUE_LIST, pts);
        break;
    // Instantiate class for Circle Marker
    case MARKER_CIRCLE:
        drawBase = (DrawBase*)new DrawCircleFixedSize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                      MODE_INT_EMPTY, MODE_ALG_DIRECT>(
            DRAW_COMM_VALUE_LIST, pts);
    ...
    case MARKER_XMARK:
        drawBase = (DrawBase*)new DrawXMarkFixedSize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                     MODE_ALG_DIRECT>(DRAW_COMM_VALUE_LIST, pts);
        break;
    // Instantiate class for Plus Marker
    case MARKER_PLUS:
        drawBase = (DrawBase*)new DrawPlusFixedSize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE>(
            DRAW_COMM_VALUE_LIST, pts);
        break;
    // Instantiate class for Star Marker
    case MARKER_STAR:
        drawBase = (DrawBase*)new DrawStarFixedSize<DT, BT, PT, VT, OpDT, OpBT, MODE_PORT_ONE,
                                                    MODE_ALG_DIRECT>(DRAW_COMM_VALUE_LIST, pts);
    default:;
    }
