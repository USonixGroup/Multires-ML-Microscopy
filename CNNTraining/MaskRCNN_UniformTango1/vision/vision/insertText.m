...
%                  Default: "black"
%
%  TextBoxColor    Color of the text box. Specify the color in the same way
%                  as the FontColor.
%   
%                  Default: "yellow"
%
...
%  RGB matches the class of I. The classes of FontColor and TextBoxColor can
%  also be uint8, uint16, int16, double or single.
%
...
%  RGB = insertText(RGB, [256 50], OWithMacron, TextBoxColor="w");
%  figure
%  imshow(RGB)
...
%         TextBoxColor=box_color, BoxOpacity=0.4);
%  figure
%  imshow(RGB)
%  title("Board")
%
...
crossCheckFontSize(font, fontSize);
...
if isempty(parser)||isempty(oldInpClass)||(~strcmp(oldInpClass, inpClass))
  ...
  parser.addParameter('TextColor', defaults.TextColor);  % Old PV pair that's not to be used as of 23b release
  parser.addParameter('FontColor', defaults.TextColor);
  parser.addParameter('BoxColor', defaults.BoxColor);    % Old PV pair that's not to be used as of 23b release
  parser.addParameter('TextBoxColor', defaults.BoxColor);
...
% Which options were specified for text/font color?
textColor = vision.internal.parseInserterColor("TextColor", "FontColor", parser, mfilename, inpClass);
boxColor = vision.internal.parseInserterColor("BoxColor", "TextBoxColor", parser, mfilename, inpClass);

boxOpacity  = double(parser.Results.BoxOpacity);
font        = vision.internal.checkFont(parser.Results.Font, mfilename);% readjusted case in font name
...
boxColor = vision.internal.codegen.parseInserterColor('BoxColor', 'TextBoxColor', ...
    optarg, coder.const(mfilename), inpClass, defaults, varargin{:});
...
%==========================================================================
function crossCheckFontSize(fontName, fontSize)
        ...
        for i=1:numPLanes
            for c=startC:endC
                for r=startR:endR
                    ...
                    tmp11 =  eml_cast(tmp1, cClassName,'to zero','spill');
                    tmp22 =  eml_cast(tmp2, cClassName,'to zero','spill');                    
...
if isSimMode
    ...
    dummyOut = int32(0);
    libMethodName = coder.internal.getCLibName('sprintf');
    dummyOut = coder.ceval(libMethodName, coder.ref(str1), cstring('%0.5g'), numDbl); %#ok<NASGU>
    str = mstring(str1);
end
