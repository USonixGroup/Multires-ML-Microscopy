classdef CheckerboardDetector < vision.calibration.PatternDetector
    % CheckerboardDetector Interface for checkerboard detection for monocular camera calibration.
    %
    %   CheckerboardDetector specifies the interface for a checkerboard
    %   detector to be used for single camera calibration in the Single Camera
    ...
    %
    %   See also cameraCalibrator, detectCheckerboardPoints,
    ...
    properties(Constant)
        ...
        % VisualGuide Light Mode
        CheckerBoardImgLight =  fullfile(toolboxdir('vision'),'vision', ...
                                '+vision', '+internal', '+calibration', ...
                                '+tool', 'CheckerboardVisualGuideLight.png');

        % VisuaGuide Dark Mode
        CheckerBoardImgDark = fullfile(toolboxdir('vision'),'vision', ...
                               '+vision', '+internal', '+calibration', ...
    ...
    methods
        ...
        function [imagePoints, imagesUsed] = detectPatternPoints(this, imageFileNames, varargin)
            ...
            [imagePoints, boardSize, imagesUsed, userCanceled] = detectCheckerboardPoints(...
    ...
    methods (Access = private)
        ...
        function configureUIComponents(this)
            ...
            propertyGridLayout = uigridlayout(this.Panel);
            propertyGridLayout.RowHeight = {'fit'};
            propertyGridLayout.ColumnWidth = {'3x','1x'};

            this.PropertyPanelGridLayout = uigridlayout(propertyGridLayout, ...
                [5 3], 'Padding', 2);
            this.PropertyPanelGridLayout.RowHeight = {'fit', 30, 1, 'fit', 'fit'};
            this.PropertyPanelGridLayout.ColumnWidth = {'fit','1x', '1x'};

            % Label
            uilabel('Parent',this.PropertyPanelGridLayout,...
                'HorizontalAlignment', 'Left',...
                'Text', vision.getMessage('vision:caltool:SquareSize'));

            % Editbox
            this.SquareSizeEditBox = uieditfield(this.PropertyPanelGridLayout,'Editable','on', ...
                'HorizontalAlignment', 'left', ...
            ...
            this.SquareSizeEditBox.Layout.Row = 1;

            % Units selection
            this.UnitsPopup = uidropdown(this.PropertyPanelGridLayout,...
            ...
            % Radio buttons
            this.RadioButtonGroup = uibuttongroup('Parent', this.PropertyPanelGridLayout, ...
                'Visible', 'on', ...
                'Units', 'pixels', ...
                'BorderType', 'none', ...
            ...
            this.PropertyImgGridLayout = uigridlayout(propertyGridLayout, 'Padding', 2);
            this.PropertyImgGridLayout.RowHeight = {'fit'};
            this.PropertyImgGridLayout.ColumnWidth = {'fit'};

            if this.isDarkMode
                visualGuideFile = this.CheckerBoardImgDark;
            else
                visualGuideFile = this.CheckerBoardImgLight;
            end
            
            vision.internal.calibration.tool.displayPatternThumbnail(this.PropertyImgGridLayout, ...
                visualGuideFile);

            addAdvancedPropertiesAccordion(this);
        end

        %------------------------------------------------------------------
        function addAdvancedPropertiesAccordion(this)
            accordionParent = matlab.ui.container.internal.Accordion('Parent', ...
                this.PropertyPanelGridLayout);
            ...
            gridLayout.ColumnWidth =  {'fit', 55, 'fit', 'fit'};
    ...
    methods
        ...
        function initializePropertyValues(this)
            ...
            if this.SquareSize <= 0 || isnan(this.SquareSize)
                % Attach uialert to Parent AppContainer
        ...
        function doSquareSizeChanged(this)
            squareSizeValue = str2double(get(this.SquareSizeEditBox,'Value'));

            if squareSizeValue <= 0 || isnan(squareSizeValue)
                % Attach uialert to Parent AppContainer
                parent = this.Panel.Parent.Parent;
                uialert(parent, getString(message('vision:caltool:invalidSquareSize')),...
                    getString(message('MATLAB:uistring:popupdialogs:ErrorDialogTitle')));
                this.SquareSizeEditBox.Value = num2str(this.SquareSize);
            else
                this.SquareSize = squareSizeValue;
            end
        end
