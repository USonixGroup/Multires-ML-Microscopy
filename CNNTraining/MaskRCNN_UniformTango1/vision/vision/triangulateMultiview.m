...
%
%   Example: Scene reconstruction from multiple views
%   -------------------------------------------------
%   % Load images
...
%   hold off
%
%   See also triangulate, imageviewset, pointTrack, matchFeatures, rigidtform3d,
...
function [worldPoints, reprojectionErrors, validIndex] = triangulateMultiview(pointTracks, ...
    camPoses, intrinsics)
...
camViewIds = zeros(numCameras, 1, 'uint32');

outputType = class(pointTracks(1).Points);

for i = 1:numCameras
    id = camPoses{i, 'ViewId'};
    camViewIds(i) = uint32(id);
    if hasAbsolutePose
    ...
    if numel(intrinsics) > 1 % cameraIntrinsics type
        cameraMatrices{id} = cast((intrinsics(i).K * [R, -R*t'])', outputType);
    else % cameraIntrinsics or cameraParameters type
        cameraMatrices{id} = cast((intrinsics.K * [R, -R*t'])', outputType);
    end
end

worldPoints = vision.internal.triangulateMultiViewPoints(pointTracks, cameraMatrices, camViewIds);
