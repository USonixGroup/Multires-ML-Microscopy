...
%classifyVideoFile Classify a video file using the Inflated-3D video classifier.
%
...
%   classifySequence.
%
%   label = classifyVideoFile(i3d, videoFilename) predicts the class label
%   for the video file, videoFilename, using the inflated3dVideoClassifier,
%   i3d. The output label is a scalar categorical. The input videoFilename
...
%   object.
%
%   [label, score] = classifyVideoFile(i3d, videoFilename) additionally
...
%   label = classifyVideoFile(i3d, videoFilename, 'NumSequences', numSequences)
%
...
%   Copyright 2021-2023 The MathWorks, Inc.
    ...
    % Setting this to true, creates new optical flow object for every
    % sequence. Unlike updateSequence/classifySequence, classifyVideoFile
    % needs a new optical flow object for every sequence.
    useFirstFrameInData = true;

    ds = transform(ds,@(data)preprocess(this,data,useFirstFrameInData));
    ...
    ii = 1;
    while hasdata(mbq)
        [args{1:numOutputs}] = next(mbq);
        [scoreOutputs{1:numOutputs}] = predict(this,args{:});
        if numOutputs == 2
            score = (scoreOutputs{1} + scoreOutputs{2}) ./2;
        else
            score = scoreOutputs{1};
        end
...
%------------------------------------------------------------------
function batchDim = iFindBatchDim(scores)
    batchDim = dims(scores{1}) == 'B';
    batchDim = find(batchDim);
end
