...
if nargin == 0 % handle mask visibilities
  ...
  intensityFlag = 0;
  % Add imageports drop down for BGR -> intensity
  if contains(conversion,'intensity')
    maskVis{imagePorts_idx} = 'on';
    intensityFlag = 1;
  end

  % Remove imageports drop down for BGR <-> RGB
  if contains(conversion,'B''G''R')
      if(~intensityFlag)
        maskVis{imagePorts_idx} = 'off';
    ...
    if isND % One multidimensional signal
      ...
      switch conversion
       ...
       case 'R''G''B'' to intensity'
        ...
        [so(1:3).port] = deal(1,1,1);
        [so(1:3).txt]  = deal('I''');
       case 'B''G''R'' to intensity'
        [si(1:3).port] = deal(1,2,3);
        [si(1:3).txt]  = deal('B''','G''','R''');
        [so(1:3).port] = deal(1,1,1);
        [so(1:3).txt]  = deal('I''');
