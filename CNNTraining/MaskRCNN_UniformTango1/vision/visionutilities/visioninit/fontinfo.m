...
allFonts = addLucidaFont(allFonts);
...
if ismac
 ...
 theFontDirs = unixFontDirs;
  % On Linux, System fonts can be located on this path:
 theFontDirs{end+1} = '/usr/share/fonts/truetype/liberation';
end
...
%-------------------------------------------------------------------------
function withLucidaFont = addLucidaFont(fontList)
withLucidaFont = fontList;
if exist('java.lang.System','class') ~= 8
    return;
end

lucidaFontDir = fullfile(toolboxdir('vision'),'visionutilities','visioninit','+fonts');
listing = dir(lucidaFontDir);

for i=1:length(listing)
    ttfStart = strfind(listing(i).name, '.ttf');
    if ~ listing(i).isdir && ~ isempty(ttfStart)
        newIndex = length(withLucidaFont) + 1;
        withLucidaFont(newIndex).fontName = listing(i).name(1:ttfStart-1);
        withLucidaFont(newIndex).fontFile = [lucidaFontDir filesep listing(i).name];
        withLucidaFont(newIndex).faceIndex = uint8(0);
    end
end
