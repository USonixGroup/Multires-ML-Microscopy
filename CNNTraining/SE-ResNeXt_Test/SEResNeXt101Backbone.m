function net = SEResNeXt101Backbone(inputSize, cardinality, baseWidth)
...
for stageIdx = 1:length(configs)
    ...
    for blockIdx = 1:config.blocks
        ...
        mainPath = [
            ...
            groupNormalizationLayer(numGroups, 'Name', [baseName '_gn1'])  % Group Norm
            ...
            groupNormalizationLayer(numGroups, 'Name', [baseName '_gn2'])  % Group Norm
            reluLayer('Name', [baseName '_relu2'])
            
            convolution2dLayer(1, config.channels, 'Name', [baseName '_conv3'], 'Padding', 'same')
            groupNormalizationLayer(numGroups, 'Name', [baseName '_gn3'])];  % Group Norm
        ...
        if stride ~= 1 || currentChannels ~= config.channels
            shortcut = [
                ...
                groupNormalizationLayer(numGroups, 'Name', [baseName '_shortcut_gn'])];  % Group Norm
