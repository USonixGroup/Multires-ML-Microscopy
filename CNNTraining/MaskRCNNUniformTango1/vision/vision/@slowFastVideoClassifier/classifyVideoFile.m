...
%classifyVideoFile Classify a video file using the SlowFast video classifier.
%
%   classifyVideoFile is recommended for applications where the entire video
%   can be classified with one label. For example, to evaluate the performance
%   of the classifier on a collection of ground truth video files. If the
%   video contains sequences with different class labels, use classifySequence.
%
%   label = classifyVideoFile(sf, videoFilename) predicts the class label
%   for the video file, videoFilename, using the slowFastVideoClassifier,
%   sf. The output label is a scalar categorical. The input videoFilename is
%   a character vector or a string scalar specifying the video file for
%   classification. The video file must be readable by a VideoReader object.
%
%   [label, score] = classifyVideoFile(sf, videoFilename) additionally returns
%   the classification score associated with the label. The score value is
%   between 0 and 1 and represents the confidence of the predicted class label.
%
%   [...] = classifyVideoFile(..., Name, Value) specifies
%   additional name-value pairs described below:
%
%   'NumSequences'
%
%        Specify the maximum number of sequences to sample from the video file.
%        Valid options are 'auto' or a positive scalar integer, numSequences.
%
%           'auto'       - The number of sequences is twice the number of
%                          sequences that is possible from the video file based on
%                          number of frames available. This provides uniform
%                          coverage over the entire video.
%
%           numSequences - Specify the maximum number of clips to select from the
%                          video file. Increase the value to ensure a more uniform
...
%        sequences are grouped into mini-batches and processed as a batch to
%        improve computational efficiency. Larger mini-batch sizes lead to
%        faster processing, at the cost of more memory.
%
%        Default: 4
%
...
%        Valid values are:
%
%           'auto' - Use a GPU if it is available, otherwise use
%                    the CPU.
%
%           'gpu'  - Use the GPU. To use a GPU, you must have
%                    Parallel Computing Toolbox(TM), and a
%                    supported GPU. If a suitable GPU
%                    is not available, an error message is
%                    issued.
%
%           'cpu'   - Use the CPU.
%
...
%      T frames, the partial video sequence is repeated to form the required
%      size.
%
%   Example: Classify a video file.
%   -------------------------------
%
%   % Load a pretrained slowfast video network.
%   sf = slowFastVideoClassifier();
%
%   % Specify the video file name to classify.
%   videoFilename = 'pushup.mp4';
%
%   % Choose 5 randomly selected video sequences to classify the video.
%   numSequences = 5;
%
%   % Classify the video using the SlowFast video classifier.
%   label = classifyVideoFile(sf, videoFilename, 'NumSequences', numSequences)
%
