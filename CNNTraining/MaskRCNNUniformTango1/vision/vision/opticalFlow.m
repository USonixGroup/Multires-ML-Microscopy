...
%#codegen
    ...
    methods (Access='public')
        ...
        function varargout = plot(this, varargin)
            ...
            %                       Default: [1 1]
            %
            %   'Color'             Specify the color of the optical flow vectors using
            %                       a color name or a 1-by-3 RGB triplet
            ...
            quiver(h, X(:), Y(:), tmpVx(:), tmpVy(:), ...
                AutoScale="on", AutoScaleFactor=inputs.ScaleFactor, ...
                Color=inputs.Color);
...
%--------------------------------------------------------------------------
function colors = iValidateAndManageColor(colors,paramName)
% Validate and manage colors. Return colors as floating point RGB triplets.
if(iscell(colors) || isempty(colors))
    error(message('vision:OpticalFlow:InvalidColor'));
end
validateattributes(colors,{'numeric','string','char'},{},paramName,mfilename);
...
% Parsing Color parameter.
inputs.Color = iValidateAndManageColor(parser.Results.Color,'Color');
...
function  checkScaleFactor(scaleFactor)
    ...
    {'nonempty', 'nonsparse', 'scalar', 'real', 'finite', 'nonnan', 'nonzero', 'positive'}, ...
