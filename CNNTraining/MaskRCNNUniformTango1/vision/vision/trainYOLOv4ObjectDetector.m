...
%  trains a YOLO v4 deep learning object detector. A YOLO v4 object detector
%  can be trained to detect multiple object classes.
%
% [..., info] = trainYOLOv4ObjectDetector(...) additionally returns
...
%
%  detector             A YOLO v4 object detector returned by the
%                       yolov4ObjectDetector function containing the network
%                       and other required parameters. See
%                       <a href="matlab:doc yolov4ObjectDetector">yolov4ObjectDetector documentation</a> for more details.
%
%  options              training options returned by the trainingOptions
%                       function in Deep Learning Toolbox. The training
%                       parameters of the neural network.
%
% [...] = trainYOLOv4ObjectDetector(..., Name=Value) specifies additional
...
%  Notes:
% ---------------
% - Custom YOLO v4 object detector created using 
%   yolov4ObjectDetector(net,classes,aboxes) object does not support
%  'FreezeSubNetwork' with value "backbone" or "backboneAndNeck".
% - Any YOLO v4 object detector created before MATLAB® ''R2024a'' 
...
% Resume training
%  ---------------
%  [...] = trainYOLOv4ObjectDetector(trainingData,checkpoint,options)
%  resumes training the YOLO v4 object detector. Use this syntax to
...
%  inorder to save detector checkpoints at regular intervals. You may resume
%  training from any one of these saved checkpoints.
%
%  See also yolov4ObjectDetector,  trainYOLOv2ObjectDetector,
...
function [trainedDetector, info] = trainYOLOv4ObjectDetector(trainingData, detector, options, varargin)
% Requires deep learning toolbox.
vision.internal.requiresNeuralToolbox(mfilename);
vision.internal.cnn.WarningLogger.initialize();

% Validate detector and trainingData.
...
inpSize = params.InputSize;
cdsTransformed = transform(trainingData, @(x) yolov4ObjectDetector.preprocessInput(x,inpSize));
...
if ~isempty(options.ValidationData)
    valDS = iValidateValData(detector,options.ValidationData);
    cdsValTransformed = transform(valDS, @(x) yolov4ObjectDetector.preprocessInput(x,inpSize));
    options.ValidationData = cdsValTransformed;
end
...
mbq = minibatchqueue(...
    ...
    "MiniBatchFcn",@helperCreateBatchData,...
...
% loss.
lossParams.NumOutputs = params.NumOutputs;
lossParams.AnchorBoxes = params.AnchorBoxes;
lossParams.InputSize = params.InputSize;
...
if ~isempty(options.Metrics)
    ...
    if (length(specifiedMetrics) > 1)
        error(message('vision:yolo:unsupportedMetricForYOLOv4'));
...
%--------------------------------------------------------------------------
function [XTrain,labelsBatched] = helperCreateBatchData(groundTruthImages,groundTruthBoxes,groundTruthClasses)
...
% Loss function.
%--------------------------------------------------------------------------
function lossVal = calculateLoss(params,isRotatedBox,varargin)
...
YPredCell = yolov4ObjectDetector.yolov4Transform(YPredictions, isRotatedBox, params.AnchorBoxes);
...
gatheredPredictions(:,2:5) = yolov4ObjectDetector.anchorBoxGenerator(params.AnchorBoxes,gatheredPredictions(:,2:5),params.InputSize);
...
for numPred = 1:size(YPredCellGathered,1)
    ...
    % Pad additional dimension to gridSize if required.
    padSize = 4-numel(gridSize);
    padMat = ones(1,padSize);
    gridSize = [gridSize padMat];
...
%--------------------------------------------------------------------------
function [trainingData, detector, isRotatedBox, params] = iParseInputsYOLOv4(trainingData,detector,options,mfilename,varargin)
% Check if the detector is a yolov4ObjectDetector object.
if ~isa(detector,'yolov4ObjectDetector')
    error(message('vision:yolo:invalidDetector'));
end

% Check if the options is a TrainingOptions object.
if ~isa(options,'nnet.cnn.TrainingOptions')
    error(message('vision:yolo:invalidOptions'));
end
...
if isa(trainingData,'matlab.io.Datastore') || isa(trainingData,'matlab.io.datastore.Datastore')
    opt.AllowRotatedBoxes = true;
    opt.AllowEmptyBoxes = true;
    opt.CheckLabels = 'auto';
    opt.IsYOLO = 'true';
    gTruthClassNames = vision.internal.inputValidation.checkGroundTruthDatastore(trainingData, opt);
else
...
if ~(isa(sampleImg,'uint8') || isa(sampleImg,'uint16') ||...
        isa(sampleImg,'int16') || isa(sampleImg,'double') ||...
        isa(sampleImg,'single') || isa(sampleImg,'gpuArray'))
    error(message('vision:yolo:unsupportedTrainInputClass'));
end
...
p.addParameter('FreezeSubNetwork', "none");
parse(p, varargin{:});
userInput = p.Results;

params.AnchorBoxes = detector.AnchorBoxes;
params.ClassNames = detector.ClassNames;
params.NumClasses = numel(detector.ClassNames);
params.NumOutputs = numel(detector.Network.OutputNames);
params.OutputNames = detector.Network.OutputNames;
params.InputSize = detector.InputSize;
params.ExperimentMonitor = iValidateExperimentMonitor(userInput,mfilename);
params.ModelName = detector.ModelName;
params.FreezeSubNetwork = iValidateFreezeSubNetwork(userInput);
end


%--------------------------------------------------------------------------
function freezeStr = iValidateFreezeSubNetwork(userInput)
if isStringScalar(userInput.FreezeSubNetwork) || ischar(userInput.FreezeSubNetwork)
    userInput.FreezeSubNetwork = validatestring(userInput.FreezeSubNetwork, ["backbone", "backboneAndNeck", "none"], mfilename, "FreezeSubNetwork");
else
    error(message('vision:yolo:invalidFreezeSubNetworkValue'));
end
freezeStr = userInput.FreezeSubNetwork;
end
%--------------------------------------------------------------------------

%--------------------------------------------------------------------------
function valData = iValidateValData(detector,valData)
% Check if the validation data is valid.
if isa(valData,'matlab.io.Datastore') || isa(valData,'matlab.io.datastore.Datastore')
    opt.AllowRotatedBoxes = true;
    gTruthClassNames = vision.internal.inputValidation.checkGroundTruthDatastore(valData, opt);
else
    error(message('vision:yolo:invalidValInput'))
end

% Copy and reset the given datastore, so external state events are
% not reflected.
valData = copy(valData);
reset(valData);
data = read(valData);
if istable(data)
    sampleData = data{1,1};
...
if ~(isa(sampleImg,'uint8') || isa(sampleImg,'uint16') || ...
        isa(sampleImg,'int16') || isa(sampleImg,'double') || ...
        isa(sampleImg,'single') || isa(sampleImg,'gpuArray'))
    error(message('vision:yolo:unsupportedValInputClass'));
end

% Check if the ground truth classes matched with the detector classes.
detectorClassNames = cellstr(detector.ClassNames);
if ~isequal(sort(gTruthClassNames)',sort(detectorClassNames))
    error(message('vision:yolo:validationClassesMismatch'));
end
...
%--------------------------------------------------------------------------
function monitor = iValidateExperimentMonitor(userInput,name)
% Validate experiment monitor input.
if (ischar(userInput.ExperimentMonitor) || isstring(userInput.ExperimentMonitor)) && strcmp(userInput.ExperimentMonitor, 'none')
    monitor = [];
else
...
detector = yolov4ObjectDetector(network,classes,anchorBoxes,PredictedBoxType=predictedBoxType,ModelName=modelName);
...
elseif ~strcmp(params.FreezeSubNetwork,"none") && ~isempty(detector.DetectionNetworkSource)
    ...
    if strcmp(params.FreezeSubNetwork,"backbone")
        error(message('vision:yolo:freezingBackboneNotSupported'));
    else
        error(message('vision:yolo:freezingBackboneAndNeckNotSupported'));
    end
end
end
...
printer.print('*************************************************************************\n');
printer.printMessage("vision:yolo:trainingHeader");
printer.linebreak;
