...
%     Semi-Global Matching and Mutual Information." International
...
%--------------------------------------------------------------------------
optBM.invalidDisparityValue = -realmax('single');
%--------------------------------------------------------------------------
...
if isSimMode()
    ...
    if strcmpi(r.Method,'SemiGlobal')
        disparityMap = vision.internal.buildable.disparitySGBMLegacyBuildable.disparitySGBM_compute(...
            I1_u8, I2_u8, optSGBM);
    else
...
skipCheck = isscalar(r.DistanceThreshold) && ...
    (r.DistanceThreshold == defaults.DistanceThreshold);
if ~skipCheck
