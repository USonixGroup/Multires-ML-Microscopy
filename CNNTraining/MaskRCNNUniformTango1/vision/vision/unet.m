function [network, outputSize] = unet(inputSize, numClasses, options)
...
arguments
    ...
    options.EncoderDepth {mustBeNumeric, mustBeScalarOrEmpty, mustBeReal, mustBeFinite, mustBeInteger, mustBeNonsparse, mustBePositive} = 4
    options.NumFirstEncoderFilters {mustBeNumeric, mustBeScalarOrEmpty, mustBeReal, mustBeFinite, mustBeInteger, mustBeNonsparse, mustBePositive} = 64
    options.FilterSize {iValidateFilterSize} = [3 3]
...
% Add channel dimension if only H and W are given for input size.
if numel(inputSize) == 2
    % The number of image channels is 1 if only [H W] vector is specified.
    inputSize = [inputSize 1];
end

% Set convolutional filter size.
if isscalar(options.FilterSize)
    convFilterSize = [options.FilterSize options.FilterSize];
else
...
if isempty(options.EncoderNetwork)
    ...
    if isa(encoder.Layers(1),"nnet.cnn.layer.ImageInputLayer") && ...
...
if strcmp(convolutionPadding, "same")
    ...
    % The UNet paper imposes a constraint that input of each 2x2 max-pooling
...
finalNumChannels = initialEncoderNumChannels * 2^(encoderDepth-1);
...
encoderBlock = [
        iCreateAndInitializeConvLayer(convFilterSize, encoderNumChannels, ...
...
encoderBlock = [encoderBlock; maxPooling2dLayer(2,Stride=2,Name=[num2str(block) '-MaxPool'])];
...
encoderDecoderBridge = [
    iCreateAndInitializeConvLayer(convFilterSize, ...
...
% Add convolutional and activation layers for each encoder block.
decoderBlock = [
        iCreateAndInitializeUpConvLayer(upConvFilterSize, ...
            decoderNumChannels, [num2str(block) '-UpConv']);
        reluLayer(Name=[num2str(block) '-UpReLU']);
        iCreateAndInitializeConvLayer(convFilterSize, decoderNumChannels, ...
        [num2str(block) '-Conv-1'], convolutionPadding);
        reluLayer(Name=[num2str(block) '-ReLU-1']);
        iCreateAndInitializeConvLayer(convFilterSize, decoderNumChannels, ...
        [num2str(block) '-Conv-2'], convolutionPadding);
...
idx = arrayfun(@(x)isa(x,'nnet.cnn.layer.Convolution2DLayer'),encoder.Layers);  
convIdx = find(idx);
...
for i = 1:length(convIdx)
    ...
    convLayer = convolution2dLayer(oldConvLayer.FilterSize,...
...
if ~isa(encoder.Layers(1),"nnet.cnn.layer.ImageInputLayer")
    inputLayer = imageInputLayer(inputSize,Name="encoderImageInputLayer");
