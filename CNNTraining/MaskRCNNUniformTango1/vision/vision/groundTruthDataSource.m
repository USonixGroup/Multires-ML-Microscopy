classdef groundTruthDataSource < vision.internal.EnforceScalarHandle & matlab.mixin.CustomDisplay
    ...
    %   Image Datastore
    %   ---------------
    %   gtSource = groundTruthDataSource(imds) returns a
    %   groundTruthDataSource object for an imageDatastore source specified
    %   in imds.
    %
    ...
    %   in imageFiles. imageFiles is a string vector or cell array of
    %   character vectors specifying image filenames.
    %
    %   Video file
    %   ----------
    %   gtSource = groundTruthDataSource(videoName) returns a
    %   groundTruthDataSource object from a video file. videoName is a
    %   scalar string or character vector containing the name of the video
    %   file.
    %
    ...
    %   image within the sequence. Images from the folder are loaded in the
    ...
    %
    %
    %   Example: Create data source from imageDatastore
    %   -----------------------------------------------
    %   % Get image file names for collection
    %   imageDir = fullfile(matlabroot, 'toolbox', 'vision', ...
    %       'visiondata','bookCovers');
    %   imds = imageDatastore(imageDir);
    %
    %   % Create data source from imageDatastore
    %   dataSource = groundTruthDataSource(imds);
    %
    %   % Read an image from the datastore and display it
    ...
    properties (SetAccess = private, Hidden)
        ...
        Reader    
        %Property to query to check if input is a datastore
        IsInputDatastore
    end
    ...
    methods
        ...
        function sourceTypeEnum = get.SourceType(this)
            ...
            elseif isa(this.Reader, 'matlab.internal.VideoReader')
                ...
                % Image Collection/Image Datastore
    ...
    methods(Hidden)
        ...
        function TF = isImageCollection(this)
            TF = isequal(this.SourceType,vision.internal.labeler.DataSourceType.ImageDatastore) && ...
                ~this.IsInputDatastore;
        end

        %------------------------------------------------------------------
        function TF = isImageDatastore(this)
            TF = isequal(this.SourceType,vision.internal.labeler.DataSourceType.ImageDatastore) && ...
                this.IsInputDatastore;
        end
    ...
    methods (Access = protected)
        ...
        function header = getHeader(this)
            ...
            if isImageSequenceSource(this)
                sourceTypeString = [' ', vision.getMessage('vision:groundTruthDataSource:imageSeqHeaderString')];
            elseif isVideoFileSource(this)
                sourceTypeString = [' ', vision.getMessage('vision:groundTruthDataSource:videoHeaderString')];
            elseif isImageCollection(this)
                sourceTypeString = [' ', vision.getMessage('vision:groundTruthDataSource:imageCollHeaderString')];
            elseif isImageDatastore(this)
                sourceTypeString = [' ', vision.getMessage('vision:groundTruthDataSource:imageDSHeaderString')];
            elseif isCustomSource(this)
                sourceTypeString = [' ', vision.getMessage('vision:groundTruthDataSource:customSeqHeaderString')];
            else
                sourceTypeString = ' ';
            end
            ...
            if ~isscalar(this)
                ...
                headerStr = [headerStr,sourceTypeString,' ',vision.getMessage('vision:groundTruthDataSource:withPropsString')];
                header = sprintf('%s\n',headerStr);
            end
        end
        ...
        function displayScalarObject(this)
            ...
            elseif isImageDatastore(this)
                sourceDisp = getImageDatastorePropertyDisplay(this);
            else
                sourceDisp = getVideoPropertyDisplay(this);
            end
            
            if this.isImageCollection || this.isImageDatastore
        ...
        %------------------------------------------------------------------
        function sourceDisp = getImageDatastorePropertyDisplay(this) %#ok<MANU>

            % Get imageDatastore display
            imdsDisp = evalc('disp(this.Reader)');
            imdsDisp = strsplit(imdsDisp, '\n');

            % Display Files, Folders and the ReadFcn properties of the
            % imageDatastore
            stopLine = find(~cellfun(@isempty, strfind(imdsDisp, '}')), 2);
            imdsDisp{1} = sprintf('\tSource:%s', imdsDisp{1});
            sourceDisp = [imdsDisp(1:stopLine(end)), imdsDisp(end-1)];
        end
        ...
        function sourceDisp = getImageCollectionPropertyDisplay(this) %#ok<MANU>
            ...
            imdsDisp = evalc('disp(this.Reader)');
            imdsDisp = strsplit(imdsDisp, '\n');
            startLine = find(~cellfun(@isempty, strfind(imdsDisp, 'Files:'))); %#ok<*STRCLFH>
    ...
    methods (Access = private)
        ...
        function inputs = parseAndPopulateInputs(this, varargin)
            ...
            inSignal = varargin{1};
            if iscell(inSignal)
                framesDeletedFlags = cellfun(@isempty, inSignal);
                inSignal(framesDeletedFlags) = [];
                varargin{1} = inSignal;
            end
        ...
        function initReaderAndPopulateTimestamps(this, sourceName, customReaderFunction, timestamps)
            ...
            % Now sourceName is either a char vector containing a
            % video/image file name or directory name or a cellstr 
            % containing image file names.

            % Check if it is an image file.
            isImageFormat = false;
            try
                % Check if imfinfo recognizes this file as an image.
                imfinfo(sourceName);
                isImageFormat = true;
            catch exception %#ok<NASGU>
                % It is not an image file. It could be a video file.
            end

            isVideoName = ischar(sourceName) && ~isfolder(sourceName) &&...
                isempty(customReaderFunction) && ~isImageFormat;
            isCustomSource = ~isVideoName && ~isempty(customReaderFunction);
            this.IsInputDatastore = isa(sourceName, 'matlab.io.datastore.ImageDatastore');
            ...
            try
                ...
                elseif isCustomSource
                    ...
                    % If timestamps are not sorted, sort timestamps.
                    ...
                    if this.IsInputDatastore
                        imgDataStore = sourceName;
                        isImageSequence = false;
                    else
                        if ~isfolder(sourceName)
                            % Checking if it is not a folder and if the
                            % extension is empty, then an empty extension will be used.
                            [~,~,ext] = fileparts(sourceName);
                            if iscell(sourceName)
                                % Check for empty char arrays in extentions
                                isEmptyExt = cellfun(@(c) ischar(c) && isempty(c), ext);
                            else
                                isEmptyExt = isempty(ext);
                            end

                            if isEmptyExt
                                imgDataStore = vision.internal.customizedImageDatastore(sourceName,ext);
                            else
                                imgDataStore = vision.internal.customizedImageDatastore(sourceName);
                            end
                        else
                            imgDataStore = vision.internal.customizedImageDatastore(sourceName);
                        end

                        isDirName = ~iscell(sourceName) && isscalar(cellstr(sourceName)) && isfolder(sourceName);
                    ...
                    if isImageSequence
                        ...
                        if isempty(timestamps)
                            ...
                            % If timestamps are not sorted, sort timestamps
                            % and re-create datastore in the correct order.
                            if ~issorted(timestamps)
                                [timestamps,idx] = sort(timestamps);
                                imgDataStore = imageDatastore(imgDataStore.Files(idx));
                            end
        ...
        function validateSource(~, srcName)
            ...
            validateattributes(srcName, {'char', 'string', 'cell', 'matlab.io.datastore.ImageDatastore'}, {'nonempty'}, mfilename, 'Data Source Name (Image Sequence, Video Name or Custom Source Name)', 1);
