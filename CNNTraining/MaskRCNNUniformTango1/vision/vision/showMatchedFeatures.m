...
% g3323648: handle malformed cell arrays of strings - {[""],[""],["--g"]}
if isa(horzcat(lineSpec{:}), 'string')
    lineSpec = horzcat(lineSpec{:});
end
...
validateattributes(options{1},{'char','string'},{},mfilename,'MarkerStyle1');
validateattributes(options{2},{'char','string'},{},mfilename,'MarkerStyle2');
validateattributes(options{3},{'char','string'},{},mfilename,'LineStyle');
...
if ~isempty(style)
    if ~contains(colorSpec, style(1))
        % try the other end
        if contains(colorSpec, style(end))
            style(end) = [];
        end
    else
        style(1) = [];
