function lines = epipolarLine(f,points)
...
% 
%   LINES = epipolarLine(F,POINTS) computes the epipolar lines in I2
%   corresponding to the points, POINTS, in I1.
%
%   LINES = epipolarLine(F',POINTS) computes the epipolar lines in I1
%   corresponding to the points, POINTS, in I2.
%
%   Inputs:
%   -------
%   POINTS  An M-by-2 matrix of [x,y] coordinates (where each row contains 
%           the x and y coordinates of a point and M is the number of 
%           points) or one of the following objects:
% 
%               BRISKPoints, cornerPoints, KAZEPoints, ORBPoints, 
%               SURFPoints, SIFTPoints, or MSERRegions.
% 
%           When POINTS is a point object type, the location coordinates 
%           are used, which have the same structure as the M-by-2 strictly 
%           coordinates input defined above.
% 
%   F       A 3-by-3 fundamental matrix. If P1, a point in I1, 
%           corresponds to P2, a point in I2, 
%           then [P2,1] * F * [P1,1]' = 0.
% 
%   Output: 
%   -------
%   LINES   An M-by-3 matrix where each row has the format [A,B,C] which 
%           defines a line as A * x + B * y + C = 0.
% 
%   Class Support
%   -------------
%   F must be double or single. POINTS must be double, single, integer, 
%   BRISKPoints, cornerPoints, KAZEPoints, ORBPoints, SURFPoints, 
%   SIFTPoints, or MSERRegions.
%
%   Example
%   -------
...
outputClass = class(f);
if isa(points, 'vision.internal.FeaturePoints') || ...
        isa(points, 'MSERRegions')
    pts = points.Location;
else
    pts = points;
end
