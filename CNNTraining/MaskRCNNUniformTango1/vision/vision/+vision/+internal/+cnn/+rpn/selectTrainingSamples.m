function samples = selectTrainingSamples(imageSize, featureMapSize, params, varargin)
%
...
[targets, positiveIndex, negativeIndex] = ...
    vision.internal.rcnn.BoundingBoxRegressionModel.generateRegressionTargetsFromProposals(...
...
regionProposals = int32([regionProposals(positiveIndex, :); regionProposals(negIdx(nidx), :)]);
anchorIDs       = {int32([anchorIDs(positiveIndex) anchorIDs(negIdx(nidx))])};
anchorIndices   = {int32([anchorIndices(positiveIndex,:); anchorIndices(negIdx(nidx),:)])};

labels = {[labels(positiveIndex) labels(negIdx(nidx))]};
