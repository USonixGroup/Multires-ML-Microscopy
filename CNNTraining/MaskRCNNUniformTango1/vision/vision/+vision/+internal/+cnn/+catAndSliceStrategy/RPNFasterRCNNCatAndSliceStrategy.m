classdef RPNFasterRCNNCatAndSliceStrategy < handle
    % RPNFasterRCNNCatAndSliceStrategy A strategy for concatenating and slicing
    ...
    methods
        ...
        function [X, response] = catBatch(~, xDim, responseDim, X, leftOverX, response, leftOverResponse)
            ...
            response = iCatResponseFcnRPN(responseDim, response, leftOverResponse);
        end
        ...
        function [slicedX, slicedResponse] = sliceBatch(~, ~, ~, trimdices, X, response)
            ...
            slicedResponse = iSliceResponseFcnRPN(response, trimdices);
        end
...
%--------------------------------------------------------------------------
function response = iCatResponseFcnRPN(~, response, extras)
    ...
    response{1} = vertcat(response{1},extras{1});
end
...
%--------------------------------------------------------------------------
function response = iSliceResponseFcnRPN(response, indices)
    if isempty(response)
        return;
    end
    response{1} = response{1}(indices, :);
end
