classdef ReprojectionErrorsDisplay < vision.internal.uitools.AbstractFigureDocument
    ...
    methods
        %------------------------------------------------------------------
        function this = ReprojectionErrorsDisplay(options)
        ...
        function saveLegendPosition(this)
            hLegend = findobj(this.Figure, 'Type', 'Legend');
            if ishandle(hLegend)
        ...
        function updateSelection(this,highlightIndices)
            ...
            hObjNormal = findobj(this.Figure,...
        ...
        function doDragLine(this,~,~)
            ...
            iptPointerManager(this.Figure,'disable');
        ...
        function reactToThemeChange(this,~,~)
            setTheme(this);
            setFigureColor(this);
            setSliderColors(this);
        end
        
        %------------------------------------------------------------------
        function setSliderColors(this)
            redColor = matlab.graphics.internal.themes.getAttributeValue...
                (this.Theme, '--mw-graphics-colorSpace-rgb-red');
            if ~isempty(this.Slider)
                ax = this.Slider.Axes;
                box = findobj(ax,'Tag','ThresholdBox');
                line = findobj(ax,'Tag','ThresholdLine');
                datatip = findobj(ax,'Tag','ThresholdTip');
                line.Color = redColor;
                box.FaceColor = redColor;
                datatip.Color = redColor;
            end
