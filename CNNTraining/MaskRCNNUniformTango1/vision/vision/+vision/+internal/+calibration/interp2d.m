...
switch class(inputImage)
    ...
    otherwise
        assert(false,'Unexpected inputImage datatype.');
end
...
if images.internal.useIPPLibrary
    ...
    if ~strcmp(inputClass, 'single') && ~strcmp(inputClass,'double') %#ok 
        % Round down the pixel values to maintain consistency with the
        % images.internal.builtins.remap function in the other code branch.
        outputImage = floor(outputImage);
    end
