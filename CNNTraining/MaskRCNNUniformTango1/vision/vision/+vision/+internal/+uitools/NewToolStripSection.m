...
%    createGalleryDropDownButton - Create a gallery drop-down button
%    createGalleryItem           - Create a gallery item
...
classdef NewToolStripSection < handle
    ...
    methods(Static)
        ...
        % Ensure that startVal is within the limits of range.
            startVal = min(max(startVal, range(1)), range(2));
        ...
        %------------------------------------------------------------------
        function galleryItem = createGalleryItem(icon, titleID)
            %  createGalleryItem Creates a gallery item
            galleryItem = matlab.ui.internal.toolstrip.GalleryItem(titleID, icon);
        end

        %------------------------------------------------------------------
        function button = createGalleryDropDownButton(icon, titleID, galleryPopup, tag)
            % createGalleryDropDownButton Create a gallery drop-down push button
            %   button = createGalleryDropDownButton(icon, titleID, tag)
            %   returns a gallery drop-down button. icon is the button's
            %   icon. titleID is the message catalog id of the button's
            %   caption. tag is a string used to identify the button for
            %   testing.
            button = matlab.ui.internal.toolstrip.DropDownGalleryButton(...
                galleryPopup, getString(message(titleID)), icon);
            button.Tag = tag;
            button.MaxColumnCount = 2;
        end
    end
