classdef (StrictDefaults)CascadeObjectDetector < matlab.System
    ...
    %
    %   <a href="matlab:helpview('vision','vision.CascadeObjectDetector.ClassificationModel')">A list of all available models is shown in the documentation.</a>
    %
    ...
    %   truecolor (RGB) image.
    %
    %   detectionResults = step(DETECTOR, ds) performs multi-scale object
    %   detection on images returned by the read method of the input
    %   datastore, ds. The datastore read method must return image data in
    %   the first column of a table or cell array. The output,
    %   detectionResults, is a 1-column table that holds the bounding boxes
    %   and each row holds the detection results corresponding to one image
    %   from the input datastore.
    %
    %   [...] = step(..., ROI) detects objects within the rectangular
    %   search region specified by ROI. ROI must be a 4-element vector, [x
    %   y width height], that defines a rectangular region of interest
    %   within image I. When processing images from a datastore, the same
    %   ROI is used for all images. The 'UseROI' property must be true to
    %   use this syntax.
    %
    ...
    properties(Nontunable)
        ...
        %   <a href="matlab:helpview('vision','vision.CascadeObjectDetector.ClassificationModel')">here</a> or an OpenCV XML file containing custom classification
        ...
        %
        %   See also <a href="matlab:helpview('vision','vision.CascadeObjectDetector.ClassificationModel')">Available models</a>
    ...
    properties
        ...
        %   classification model is shown <a href="matlab:helpview('vision','vision.CascadeObjectDetector.ClassificationModel')">here</a>. This property is tunable.
        %
        %   Default: 1.1
        ScaleFactor = 1.1;
        %MergeThreshold Threshold for merging colocated detections
    ...
    methods
        ...
        function set.ClassificationModel(obj,value)
            ...
            coder.internal.errorIf(~isSupportedModel(value) && ~file_exists ...
                && ~startsWith(value, '/sdcard'), 'vision:ObjectDetector:modelNotFound', value);
    ...
    methods(Access = protected)
        ...
        %------------------------------------------------------------------
        % Allow input size change during execution
        %------------------------------------------------------------------
        ...
        function bboxes = stepImpl(obj,I,varargin)
            ...
            if isDatastore(I) && isempty(coder.target)
                % Copy and reset input datastore to prevent modifying user
                % input object.
                ds = copy(I);
                reset(ds);

                % Define a transform to run detection on each image in the
                % input datastore.
                tds = transform(ds, ...
                    @(data, info) detectionTransform(obj, data, info, varargin{:}),...
                    'IncludeInfo', true);
                ...
                % Package results in a table.
                bboxes = table(data, 'VariableNames', {'Boxes'});

            else
                grayImage = preprocessImage(obj, I, roi);
        ...
        function loadXMLFromClassModel(obj)
            if (isSimMode())
                ...
                ClassificationModelPath = coder.internal.const(obj.getModelPath(obj.ClassificationModel));
    ...
    methods(Access = private, Hidden)
        ...
        %------------------------------------------------------------------
        function grayImage = preprocessImage(obj, I, roi)
        ...
        %------------------------------------------------------------------
        % Detection transform applies detect to each image from datastore.
        %------------------------------------------------------------------
        function [detections, info] = detectionTransform(obj, data, info, varargin)
            if isempty(coder.target)
                % Standardize data to cell array in case it is a numeric value
                % or a table.
                data = checkDatastoreOutput(data, info, 1);
                data = convertToCellIfNeeded(data);

                % Allocate output cell array.
                numImages  = size(data,1);
                detections = cell(numImages, 1);
                ...
                for i = 1:numImages
                    ...
                    try
                        I = data{i,1};
                        validateInputsImpl(obj, I, varargin{:});
                    catch ME
    ...
    methods(Static, Hidden)
        ...
        function file_path = getModelPath(name)
            coder.extrinsic('tbxprefix');
            coder.extrinsic('matlabroot');
            coder.extrinsic('fullfile');
            coder.extrinsic('which');
            
            if isdeployed && isSimMode()
                rootDirectory = coder.internal.const(tbxprefix);
            else
                rootDirectory = coder.internal.const(fullfile(matlabroot,'toolbox'));
            end
            
            dataDirectory = coder.internal.const(fullfile(rootDirectory, ...
                'vision','visionutilities','classifierdata','cascade'));
...
%--------------------------------------------------------------------------
function data = convertToCellIfNeeded(data)
if istable(data)
...
%--------------------------------------------------------------------------
function data = checkDatastoreOutput(data, info, dsRecordIdx)
...
% LocalWords:  XMLFILE BBOXES truecolor visionteam bboxes IFaces roi bbox IBody CVPR Lienhart
% LocalWords:  Kuranov Vadim Pisarevsky DAGM nonsparse Dor NGo nullcopy Iu tbxprefix visionutilities
% LocalWords:  classifierdata lbp haar haarcascade frontalface lbpcascade profileface mcs eyepair
% LocalWords:  righteye lefteye upperbody stringnot Supportedfor sdcard
