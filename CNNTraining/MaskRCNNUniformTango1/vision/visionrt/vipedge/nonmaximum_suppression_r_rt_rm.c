/*
 *  NONMAXIMUM_SUPPRESSION_R_RT_RM Helper function for Edge block (Canny method).
 *
 *  Copyright 2019 The MathWorks, Inc.
 */
#include "vipedge_rt.h"

LIBMWVISIONRT_API void MWVIP_NonMaximum_Suppression_R_RM(real32_T *dc,
                                                        real32_T *dr,
                                                        real32_T *tmpOrMag,
                                                        int_T inpRows,
                                                        int_T inpCols)
{
    ...
    for (r=1; r<inpRows-1; r++)
    {
        for (c=1; c<inpCols-1; c++)
        {
            ...
            dc_rc = dc[r*inpCols+c];
            dr_rc = dr[r*inpCols+c];
            ...
            if (fabsf(dr_rc) > fabsf(dc_rc))
            {
                ...
                mag2 = Rnorm(dc[(r-1)*inpCols+c], dr[(r-1)*inpCols+c]);
                mag4 = Rnorm(dc[(r+1)*inpCols+c], dr[(r+1)*inpCols+c]);
                if (dc_rc*dr_rc > 0)
                {
                    mag3 = Rnorm(dc[(r+1)*inpCols+(c+1)], dr[(r+1)*inpCols+(c+1)]);
                    mag1 = Rnorm(dc[(r-1)*inpCols+(c-1)], dr[(r-1)*inpCols+(c-1)]);
                }
                else
                {
                    mag3 = Rnorm(dc[(r+1)*inpCols+(c-1)], dr[(r+1)*inpCols+(c-1)]);
            ...
            else
            {
                ...
                mag2 = Rnorm(dc[r*inpCols+(c+1)], dr[r*inpCols+(c+1)]);
                mag4 = Rnorm(dc[r*inpCols+(c-1)], dr[r*inpCols+(c-1)]);
                if (dc_rc*dr_rc > 0)
                {
                    mag3 = Rnorm(dc[(r-1)*inpCols+(c-1)], dr[(r-1)*inpCols+(c-1)]);
                    mag1 = Rnorm(dc[(r+1)*inpCols+(c+1)], dr[(r+1)*inpCols+(c+1)]);
           ...
           rare event that can happen with synthetic images. g676673 */
            ...
            {
                tmpOrMag[r*inpCols+c] = 0;
                
                if ((fabs(dr_rc) > fabs(dc_rc)) && (dr[r*inpCols+c] < 0 ))      /* up-down */
                {
                    tmpOrMag[r*inpCols+c] = mag;
                }
                else if ((fabs(dr_rc) < fabs(dc_rc)) && (dc[r*inpCols+c] > 0 )) /* right-left */
                {
                    tmpOrMag[r*inpCols+c] = mag;
                }
                else
...
/* [EOF] nonmaximum_suppression_r_rt_rm.c */
