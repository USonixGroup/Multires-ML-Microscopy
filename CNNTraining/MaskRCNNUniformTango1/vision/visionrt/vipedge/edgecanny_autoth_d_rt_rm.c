/*
 *  EDGECANNY_AUTOTH_D_RT_RM Helper function for Edge block (Canny method).
 *
 *  Copyright 2019 The MathWorks, Inc.
 */
#include "vipedge_rt.h"

static void TrackNonRecursive_AD_RM ( boolean_T *edge,
                                    real_T  *tmpOrMag,
                                    int16_T   *stackR,
                                    int16_T   *stackC,
                                    int16_T   *stackP,
...
static void EstimateAutoThreshold_D_RM(real_T *tmpOrMag,
                                    real_T *highTh,
                                    real_T *lowTh,
                                    int_T inpRows,
                                    int_T inpCols,
                                    ...
                                    const real_T *autoPercent)
{
    ...
    for (r=quarterFiltLen; r<inpRows-quarterFiltLen; r++)
        ...
        {
            real_T valBasedIdx = (tmpOrMag[r*inpCols+c] * N_BINS_MIN1);
...
static void HysteresisThresholding_AD_RM (boolean_T *edge,
                                        real_T  *tmpOrMag,
                                        int16_T   *stackR,
                                        int16_T   *stackC,
                                        int16_T   *stackP,
                                        ...
                                        const  real_T  *autoPercent)
{
    ...
    EstimateAutoThreshold_D_RM (tmpOrMag, &highTh, &lowTh,inpRows,
    ...
    for (r=0; r<inpRows; r++)
        for (c=0; c<inpCols; c++)
            if (tmpOrMag[r*inpCols + c] >= highTh)
                TrackNonRecursive_AD_RM(edge,tmpOrMag,stackR,stackC,stackP,stackQ,
                        r, c, lowTh, inpRows,inpCols);
}


LIBMWVISIONRT_API void MWVIP_EdgeCanny_autoTh_D_RM(
                                                const real_T  *inpImg,
                                                ...
                                                int_T  halfFiltLen)
{
    ...
    MWVIP_RC_Gaussian_Smoothing_D_RM(inpImg,gauss1D,cFiltered,rFiltered,inpRows,inpCols,halfFiltLen);
    
    /* step-2: filter smoothed image along column with the derivative of Gaussian filter */
    MWVIP_C_Derivative_Image_D_RM(cFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    ...
    MWVIP_R_Derivative_Image_D_RM(rFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    ...
    MWVIP_NonMaximum_Suppression_D_RM(cFiltered,rFiltered,tmpOrMag,inpRows,inpCols);
    ...
    HysteresisThresholding_AD_RM (outEdge,tmpOrMag,stackR,stackC,stackP,stackQ,
...
/* [EOF] edgecanny_autoth_d_rt_rm.c */
