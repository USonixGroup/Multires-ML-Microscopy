/*
 *  NONMAXIMUM_SUPPRESSION_D_RT_RM Helper function for Edge block (Canny method).
 *
 *  Copyright 2019 The MathWorks, Inc.
 */
#include "vipedge_rt.h"

LIBMWVISIONRT_API void MWVIP_NonMaximum_Suppression_D_RM(real_T *dc,
                                                        real_T *dr,
                                                        real_T *tmpOrMag,
                                                        int_T inpRows,
                                                        int_T inpCols)
    ...
    for (r=1; r<inpRows-1; r++)
    {
        for (c=1; c<inpCols-1; c++)
        {
            ...
            if (fabs(dr_rc) > fabs(dc_rc))
            {
                ...
                mag2 = Dnorm(dc[(r-1)*inpCols+c], dr[(r-1)*inpCols+c]);
                mag4 = Dnorm(dc[(r+1)*inpCols+c], dr[(r+1)*inpCols+c]);
                if (dc_rc*dr_rc > 0)
                {
                    mag3 = Dnorm(dc[(r+1)*inpCols+(c+1)], dr[(r+1)*inpCols+(c+1)]);
                    mag1 = Dnorm(dc[(r-1)*inpCols+(c-1)], dr[(r-1)*inpCols+(c-1)]);
                }
                else
                {
                    mag3 = Dnorm(dc[(r+1)*inpCols+(c-1)], dr[(r+1)*inpCols+(c-1)]);
            ...
            else
            {
                ...
                mag2 = Dnorm(dc[r*inpCols+(c+1)], dr[r*inpCols+(c+1)]);
                mag4 = Dnorm(dc[r*inpCols+(c-1)], dr[r*inpCols+(c-1)]);
                if (dc_rc*dr_rc > 0)
                {
                    mag3 = Dnorm(dc[(r-1)*inpCols+(c-1)], dr[(r-1)*inpCols+(c-1)]);
                    mag1 = Dnorm(dc[(r+1)*inpCols+(c+1)], dr[(r+1)*inpCols+(c+1)]);
...
/* [EOF] nonmaximum_suppression_d_rt_rm.c */
