...
// Early truncation limit for coordinates before view port truncation
// at subpixel level to stop the overflow for large input coordinates.
#define MIN_TRUNCATE_COORDINATE   -10
...
{   
    ...
    if (x > (MIN_TRUNCATE_COORDINATE-1) && x <= numCol && y > (MIN_TRUNCATE_COORDINATE-1) && y <= numRow)       
     return true;    
...
template<class T>
    void getPoint(T* position, double slope , double xInter,
                  int numRows, int numCols, int colIdx, int rowIdx, boolean_T pointsOut){
    
    double p1[2], p2[2], p3[2], p4[2], validPoint1[2], validPoint2[2]; 
    validPoint1[0] = double(MIN_TRUNCATE_COORDINATE);
    validPoint1[1] = double(MIN_TRUNCATE_COORDINATE);
    validPoint2[0] = double(MIN_TRUNCATE_COORDINATE);
    validPoint2[1] = double(MIN_TRUNCATE_COORDINATE);

    p1[0] = double(MIN_TRUNCATE_COORDINATE);
    p1[1] = xInter+(MIN_TRUNCATE_COORDINATE*slope);
   
    p2[0] = (MIN_TRUNCATE_COORDINATE-xInter)/slope;
    p2[1] = double(MIN_TRUNCATE_COORDINATE);

    p3[0] = numCols;
    p3[1] = xInter+slope * (numCols);
    ...
    if (std::isinf(slope)){
        ...
        p2[0] = position[colIdx];
        p2[1] = double(MIN_TRUNCATE_COORDINATE);

        p4[0] = position[colIdx];
        p4[1] = numRows;
    }

    boolean_T visited = false;
    
    if(isPointOnLine(p1, position, slope, xInter) && isInbound(int(p1[0]), int(p1[1]), numRows, numCols))
    {   
       if (visited){
        validPoint2[0] = static_cast<T>(p1[0]);
        validPoint2[1] = static_cast<T>(p1[1]);}
        else{
        validPoint1[0] = static_cast<T>(p1[0]);
        validPoint1[1] = static_cast<T>(p1[1]);
        }
        visited = true;
    };
    if(isPointOnLine(p2, position, slope, xInter) && isInbound(int(p2[0]), int(p2[1]), numRows, numCols))
    {
       if (visited){
        validPoint2[0] = static_cast<T>(p2[0]);
        validPoint2[1] = static_cast<T>(p2[1]);}
        else{
        validPoint1[0] = static_cast<T>(p2[0]);
        validPoint1[1] = static_cast<T>(p2[1]);
        }
        visited = true;
    };
    
    if(isPointOnLine(p3, position, slope, xInter) && isInbound(int(p3[0]), int(p3[1]), numRows, numCols))
    { 
        if (visited){
        validPoint2[0] = static_cast<T>(p3[0]);
        validPoint2[1] = static_cast<T>(p3[1]);}
        else{
        validPoint1[0] = static_cast<T>(p3[0]);
        validPoint1[1] = static_cast<T>(p3[1]);
        }
        visited = true;
    }; 
    ...
    {  
        if (visited){
        validPoint2[0] = static_cast<T>(p4[0]);
        validPoint2[1] = static_cast<T>(p4[1]);}
        else{
        validPoint1[0] = static_cast<T>(p4[0]);
        validPoint1[1] = static_cast<T>(p4[1]);
        }
        visited = true;
    };

    if (pointsOut){
    position[PT_COL1] = static_cast<T>(validPoint1[0]);
    position[PT_ROW1] = static_cast<T>(validPoint1[1]);
    position[PT_COL2] = static_cast<T>(validPoint2[0]);
    position[PT_ROW2] = static_cast<T>(validPoint2[1]);}
    else{
        position[colIdx] = static_cast<T>(validPoint1[0]);
        position[rowIdx] = static_cast<T>(validPoint1[1]);
    }
}
...
template<class T>
    ...
    boolean_T tf1 = isInbound(line[PT_COL1], line[PT_ROW1], numRows, numCols);
    boolean_T tf2 = isInbound(line[PT_COL2], line[PT_ROW2], numRows, numCols);
    boolean_T ff1 = !tf1;
    boolean_T ff2 = !tf2;

    // Both points are inside the image , no truncation required    
    if (tf1 && tf2)
        return;
    ...
    boolean_T pointsOut = ff1&ff2;
    if (pointsOut){
         // If both points are outside the image, truncate the points
           getPoint(line,slope,xInter,numRows,numCols,PT_COL1,PT_ROW1, pointsOut);
    }  
    else{
        // First point is outside the image , truncate the point
        if (ff1)
            getPoint(line, slope, xInter, numRows, numCols, PT_COL1, PT_ROW1, pointsOut);

        // Second point is outside the image , truncate the point
        if (ff2)
            getPoint(line, slope, xInter, numRows, numCols, PT_COL2, PT_ROW2, pointsOut);
    }
}
...
public:
    ...
    inline void MoveToNextColumn(void)
    {
		...
		if(mStatus == CIRCLE_COUNTERCLOCKWISE){
			isContinue = (mCol >= 0);
            while(isContinue) {
                if(mErr < 0) {
                    mErr += (2 - mRowDif);
