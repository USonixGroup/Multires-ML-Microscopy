...
DecodeBitStream(ByteArray&& bytes, const bool isModel1, const Version& version, ErrorCorrectionLevel ecLevel, const std::string& hintedCharset)
{
...
DoDecode(const BitMatrix& bits, const Version& version, const FormatInformation& formatInfo, const std::string& hintedCharset)
{
	...
	// Read codewords
	ByteArray codewords = formatInfo.isModel1() ? BitMatrixParser::ReadCodewordsModel1(bits, version) : BitMatrixParser::ReadCodewords(bits, version);
	if (codewords.empty()) {
		return DecodeStatus::FormatError;
	}
	...
	return DecodeBitStream(std::move(resultBytes), formatInfo.isModel1(), version, ecLevel, hintedCharset);
}
...
Decoder::Decode(const BitMatrix& bits_, const std::string& hintedCharset)
{
	...
	const Version* version = formatInfo.isModel1() ? BitMatrixParser::ReadVersionModel1(bits, false) : BitMatrixParser::ReadVersion(bits, false);
	...
	version = formatInfo.isModel1() ? BitMatrixParser::ReadVersionModel1(bits, true) : BitMatrixParser::ReadVersion(bits, true);
