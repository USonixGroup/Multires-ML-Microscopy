...
FindRowsWithPattern(const BitMatrix& matrix, int height, int width, int startRow, int startColumn, const std::vector<int>& pattern, std::array<Nullable<ResultPoint>, 4>& result)
{
	...
	if (found) {
		...
		for (; stopRow < height; stopRow++) {
			...
			found = FindGuardPattern(matrix, previousRowStart, stopRow, width, false, pattern, counters, startPos1, endPos1);
			// a found pattern is only considered to belong to the same barcode if the start and end positions
			// don't differ too much. Pattern drift should be not bigger than two for consecutive rows. With
			// a higher number of skipped rows drift could be larger. To keep it simple for now, we allow a slightly
			// larger drift and don't check for skipped rows.
			if (found && std::abs(previousRowStart - startPos1) < MAX_PATTERN_DRIFT && std::abs(previousRowEnd - endPos1) < MAX_PATTERN_DRIFT) {
				previousRowStart = startPos1;
				previousRowEnd = endPos1;
				skippedRowCount = 0;
			}
