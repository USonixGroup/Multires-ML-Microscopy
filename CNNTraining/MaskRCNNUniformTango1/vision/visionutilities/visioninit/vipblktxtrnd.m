...
%% Compute the icon string
    ...
    case 'dynamic'
        if calledFromBlock
            blkMask = Simulink.Mask.get(gcbh);
            getTextColorFrom        = get_param(blk, 'getTextColorFrom');
            getTextLocFrom          = get_param(blk, 'getTextLocFrom');
            getTextOpacityFrom      = get_param(blk, 'getTextOpacityFrom');
            textColor        =  blkMask.getParameter('textColor');
            textOpacity      =  blkMask.getParameter('textOpacity');
            textLoc          =  blkMask.getParameter('textLoc');
            fontFace         =  blkMask.getParameter('fontFace');

            fontFace.TypeOptions = fontinfo;
            if strcmp(getTextColorFrom,'Input port')
                textColor.Visible ='off';
            else
                textColor.Visible = 'on';
            end
            if strcmp(getTextLocFrom,'Input port')
                textLoc.Visible   ='off';
            else
                textLoc.Visible   = 'on';
            end
            if strcmp(getTextOpacityFrom,'Input port')
                textOpacity.Visible ='off';
            else
                textOpacity.Visible = 'on';
            end
        end
otherwise
